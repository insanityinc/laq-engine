...
COO_krao_fbv_fbv(block* A, block* B, block* C) {
  int i = 0, j = 0;
  while (i < A->columns_size() && j < B->columns_size()) {
    if ( A->columns(i) < B->columns(j) ) {
      ++i;
    } else if ( A->columns(i) > B->columns(j) ) {
      ++j;
    } else {
      C->add_columns(A->columns(i));
      ++i;
      ++j;
    }
  }
...
move_COO_krao_fbv_bm(block* A, block* B, block* C) {
  C->mutable_rows()->Reserve(A->columns_size());
  for (auto& col : A->columns()) {
    C->add_rows(B->rows()[col]);
  }
...
COO_krao_fbv_bm(block* A, block* B, block* C) {
  ...
  *(C->mutable_columns()) = *(A->mutable_columns());
}
...
move_COO_krao_fbv_dv(block* A, block* B, block* C) {
  C->mutable_values()->Reserve(A->columns_size());
  for (auto &col : A->columns()) {
    C->add_values(B->values()[col]);
  }
  C->columns() = std::move(A->columns());
}
*/
...
move_krao_bm_bm(block* A, block* B, block* C, int rows_A) {
  #ifdef DEBUG
    assert(A->rows_size() == B->rows_size());
  #endif  // DEBUG
...
krao_bm_bm(block* A, block* B, block* C, int rows_A) {
    ...
    assert(A->rows_size() == B->rows_size());
  #endif  // DEBUG
  C->mutable_rows()->Reserve(A->rows_size());
  for (int i = 0; i < A->rows_size(); ++i) {
    C->add_rows(A->rows(i) * rows_A + B->rows(i));
  }
}
...
move_COO_krao_bm_fbm(block* A, block* B, block* C, int rows_A) {
  C->mutable_rows()->Reserve(B->columns_size());
  for (int i = 0; i < B->columns_size(); ++i) {
    int col = B->columns(i);
    C->add_rows(A->rows()[col] * rows_A + B->rows(i));
  }
...
COO_krao_bm_fbm(block* A, block* B, block* C, int rows_A) {
  ...
  *(C->mutable_columns()) = *(B->mutable_columns());
}
...
move_krao_bm_dv(block* A, block* B, block* C) {
  #ifdef DEBUG
    assert(A->rows_size() == B->values_size());
  #endif  // DEBUG
...
krao_bm_dv(block* A, block* B, block* C) {
    ...
    assert(A->rows_size() == B->values_size());
  #endif  // DEBUG
  *(C->mutable_values()) = *(B->mutable_values());
  *(C->mutable_rows()) = *(A->mutable_rows());
}
...
move_COO_krao_fbm_bm(block* A, block* B, block* C, int rows_A) {
  ...
  for (int i = 0; i < A->columns_size(); ++i) {
    int col = A->columns(i);
    C->add_rows(A->rows(i) * rows_A + B->rows()[col]);
  }
  C->columns() = std::move(A->columns());
}
*/
...
COO_krao_fbm_fbm(block* A, block* B, block* C, int rows_A) {
  ...
  while (i < A->columns_size() && j < B->columns_size()) {
    if ( A->columns(i) < B->columns(j) ) {
      ...
      C->add_rows(A->rows(i) * rows_A + B->rows(j));
      C->add_columns(A->columns(i));
      ++i;
      ++j;
    }
...
krao_dv_dv(block* A, block* B, block* C) {
    ...
    assert(A->values_size() == B->values_size());
  #endif
  C->mutable_values()->Reserve(A->values_size());
  for (int i = 0; i < A->values_size(); ++i) {
    C->add_values(A->values(i) * B->values(i));
  }
}
...
move_COO_krao_dv_fdv(block* A, block* B, block* C) {
  C->mutable_values()->Reserve(B->columns_size());
  for (int i = 0; i < B->columns_size(); ++i) {
    int col = B->columns(i);
    C->add_values(A->values()[col] * B->values(i));
  }
  C->columns() = std::move(B->columns());
}
*/
...
COO_krao_fdv_fdv(block* A, block* B, block* C) {
  ...
  while (i < A->columns_size() && j < B->columns_size()) {
    if ( A->columns(i) < B->columns(j) ) {
      ...
      C->add_values(A->values(i) * B->values(j));
      C->add_columns(A->columns(i));
      ++i;
      ++j;
    }
...
move_COO_krao_fdv_dm(block* A, block* B, block* C) {
  ...
  for (int i = 0; i < A->columns_size(); ++i) {
    int col = A->columns(i);
    C->add_values(A->values(i) * B->values()[col]);
    C->add_rows(B->rows()[col]);
  }
  C->columns() = std::move(A->columns());
}
...
krao_dm_bm(block* A, block* B, block* C, int rows_A) {
  krao_bm_bm(A, B, C, rows_A);
  *(C->mutable_values()) = *(A->mutable_values());
}
...
void
krao(block* A, block* B, block* C,
     int rows_A, bool CSC, bool move) {
  int hex = (A->values_size() >0) << 6 |
            (A->rows_size() > 0) << 5 |
            (A->columns_size() > 0) << 4 |
            (B->values_size() > 0) << 2 |
            (B->rows_size() > 0) << 1 |
